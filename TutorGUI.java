...
public class TutorGUI extends JFrame {
    ...
    private final Color CANCEL = new Color(108, 117, 125); // Darker gray for cancel buttons
    ...
    private void createUI() {
        ...
        buttonPanel.add(createButton("Add Class", PRIMARY, _ -> showAddClassDialog()));
        buttonPanel.add(createButton("Update Class", PRIMARY, _ -> showUpdateClassDialog()));
        buttonPanel.add(createButton("Delete Class", ERROR, _ -> showDeleteClassDialog()));
        buttonPanel.add(createButton("View Students", ACCENT, _ -> showStudentsDialog()));
        buttonPanel.add(createButton("Update Profile", PRIMARY, _ -> showProfileDialog()));
    ...
    private void showAddClassDialog() {
        ...
        // Set default times (9:00am to 10:00am)
        startHour.setSelectedItem("9");
        ...
        JButton saveBtn = createButton("Save", ACCENT, _ -> {
        ...
        JButton cancelBtn = createButton("Cancel", CANCEL, _ -> dialog.dispose());
    ...
    private void showUpdateClassDialog() {
        ...
        if (currentSchedule.contains(" ")) {
            ...
            if (scheduleParts.length >= 2) {
                ...
                // Parse time range (e.g., "9:00am-10:00am")
                if (scheduleParts.length >= 2) {
                    String timeRange = scheduleParts[1];
                        ...
                        if (times.length >= 2) {
                            ...
                            if (startTime.contains(":")) {
                                ...
                                if (startParts.length >= 2) {
                                    String hour = startParts[0];
                                    String minuteAmPm = startParts[1];
                                    String minute = minuteAmPm.substring(0, 2);
                                    String startAmPm = minuteAmPm.substring(2);
                                    
                                    // Debug output
                                    System.out.println("Parsing start time: " + startTime);
                                    System.out.println("Hour: " + hour + ", Minute: " + minute + ", AM/PM: " + startAmPm);
                                    
                                    startHour.setSelectedItem(hour);
                                    startMinute.setSelectedItem(minute);
                                    startAMPM.setSelectedItem(startAmPm);
                                }
                            }
                            ...
                            if (endTime.contains(":")) {
                                ...
                                if (endParts.length >= 2) {
                                    String hour = endParts[0];
                                    String minuteAmPm = endParts[1];
                                    String minute = minuteAmPm.substring(0, 2);
                                    String endAmPm = minuteAmPm.substring(2);
                                    
                                    // Debug output
                                    System.out.println("Parsing end time: " + endTime);
                                    System.out.println("Hour: " + hour + ", Minute: " + minute + ", AM/PM: " + endAmPm);
                                    
                                    endHour.setSelectedItem(hour);
                                    endMinute.setSelectedItem(minute);
                                    endAMPM.setSelectedItem(endAmPm);
                                }
                            }
                        }
                    }
        ...
        JButton updateBtn = createButton("Update", ACCENT, _ -> {
            String subject = subjectField.getText().trim();
            String chargesStr = chargesField.getText().trim();
            String day = (String) dayBox.getSelectedItem();
            String start = startHour.getSelectedItem() + ":" + startMinute.getSelectedItem() + startAMPM.getSelectedItem();
